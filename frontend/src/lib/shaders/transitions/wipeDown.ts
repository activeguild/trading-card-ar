// components/cardEffect/shaders/wipeDown.ts

export const wipeDownShader = `
  ...
  void main() {
    ...
    // Overshoot progress so the wipe edge passes the topmost pixel by progress=1.
    float p = progress * 1.05;
    float edge = smoothstep(p - 0.02, p + 0.02, 1.0 - uv.y);
