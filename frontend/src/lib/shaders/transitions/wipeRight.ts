// components/cardEffect/shaders/wipeRight.ts

export const wipeRightShader = `
  ...
  void main() {
    ...
    // Overshoot progress so the wipe edge passes the leftmost pixel by progress=1.
    float p = progress * 1.05;
    float edge = smoothstep(p - 0.02, p + 0.02, 1.0 - uv.x);
    float mask = edge * texColor.a;
